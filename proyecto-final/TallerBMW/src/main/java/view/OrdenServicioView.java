...
public class OrdenServicioView extends JPanel {
    ...
    private JPanel buildFormulario() {
        ...
        cmbVehiculo = new JComboBox<>(); cmbVehiculo.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        cmbMecanico = new JComboBox<>(); cmbMecanico.setFont(new Font("Segoe UI", Font.PLAIN, 13));
        ...
        txtCosto.putClientProperty("JTextField.placeholderText", "0.00");
        ...
        card.add(buildComboGroup("Vehiculo *", cmbVehiculo));
        card.add(Box.createVerticalStrut(8));
        card.add(buildComboGroup("Mecanico *", cmbMecanico));
        card.add(Box.createVerticalStrut(8));
        card.add(lblDesc);
    ...
    private JScrollPane buildTabla() {
        String[] columnas = {"ID", "Vehiculo", "Cliente", "Mecanico", "Estado", "Costo", "Entrada", "Recepcionista"};
        ...
        tabla.getSelectionModel().addListSelectionListener(e -> {
            ...
            if (fila >= 0) {
                ...
                setStatus("Orden #" + idSeleccionado + " — Estado: " + modeloTabla.getValueAt(fila, 4), TEXT_MUTED);
            }
        });
    ...
    private void cargarCombos() {
        try {
            ...
            setStatus("Error cargando vehiculos: " + ex.getMessage(), DANGER);
        }
        try {
            UsuarioDAO usuarioDAO = new UsuarioDAO();
            mecanicos = usuarioDAO.listarPorRol("MECANICO");
            cmbMecanico.removeAllItems();
            for (Usuario u : mecanicos) cmbMecanico.addItem(u.getNombre());
        } catch (Exception ex) {
            setStatus("Error cargando mecanicos: " + ex.getMessage(), DANGER);
        }
    }
    ...
    private void cargarTabla() {
        ...
        try {
            ...
            setStatus("Error cargando ordenes: " + ex.getMessage(), DANGER);
        }
    }
    ...
    private void agregarFila(OrdenServicio o) {
        modeloTabla.addRow(new Object[]{
                o.getId(), o.getPlacaVehiculo(), o.getNombreCliente(),
                o.getNombreMecanico() != null ? o.getNombreMecanico() : "—",
                o.getEstado(), String.format("%.2f", o.getCostoTotal()),
                o.getFechaEntrada().toLocalDate(), o.getNombreRecepcionista()
        });
    }
    ...
    private void completarOrden() {
        if (idSeleccionado == -1) { JOptionPane.showMessageDialog(this, "Selecciona una orden primero"); return; }
        int ok = JOptionPane.showConfirmDialog(this, "Marcar esta orden como completada?", "Confirmar", JOptionPane.YES_NO_OPTION);
